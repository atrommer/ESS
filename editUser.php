...
?>
	<form id="frmDelete" name="frmDelete" method="post" action="<?=$_SERVER['PHP_SELF'] ?>">
		<input type="hidden" name="hdUserID" value="<?=$_REQUEST['u_id'] ?>">
		<input type="hidden" name="confirmDelete" value="1">
		<input type="submit" name="delete" value="Are you sure you want to deactivate <? print "$aUserVals->user_first $aUserVals->user_last"; ?>?" >
	</form>
<?
}
...
<form action="<?=$_SERVER['PHP_SELF'] ?>" method="post" name="frmAddUser"><table width="100%"  border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td>Hourly Rate </td>
    <td>$<input name="tbPay" type="text" id="tbPay" size="6" maxlength="6" value="<?=$defaults['tbPay']?>"></td>
  </tr>
  <tr class="evenRow">
	<td>User Inactive?</td>
	<td>Yes:
  <input type="radio" name="cbActive" value="1"<? if ($defaults['cbActive']) print " checked=checked"; ?>/>
  	 No: <input type="radio" name="cbActive" value="0"<? if (!$defaults['cbActive']) print " checked=checked"; ?>/>
  </td>
  </tr>
  <tr>
